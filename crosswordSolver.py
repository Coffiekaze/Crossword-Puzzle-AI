    ...
    for slot in slots:
        ...
        if pattern == '.' * N:
            ...
            domain = [
                ...
                if all(p == '.' or p == w[i] for i, p in enumerate(pattern))
            ]
...
def solve_crossword(puzzle_path="puzzle.json", unique=True) -> Optional[List[str]]:
...
def main():
    args = parse_args()
    result = solve_crossword(args.puzzle, unique=args.unique)
    if result is None:
        print("No solution found for the given crossword.")
        sys.exit(1)
    for row in result:
        print(row)
